...
const Taskrecord = () => {
  ...
  const [showStatusCard, setShowStatusCard] = useState("null");
  ...
  const handleViewTaskClick = (status: SetStateAction<string>) => {
    setShowStatusCard("show_status_card");
    setCurrentStatus(status); // Set the current status based on the clicked item
  };
  ...
  return (
    <div className="w-3/4 h-screen   mt-20">
      <div className="flew flex-col p-4 justify-center items-center">
        <h1 className="font-[500] text-[24px]">Tasks Records</h1>

        <div className=" flex flex-col">
          {/* -----------------search part----------- */}

          <div className="flex w-full mt-4 items-center">
            <p>{data.length} Total Tasks Created</p>
              ...
              {getCurrentData().map((item, index) => (
                <div key={index} className="flex flex-col">
                  <div className="flex justify-between items-center gap-4 w-full">
                    ...
                    <div
                      className="w-[121px] h-[30px] p-1 text-center border text-[#007F00] border-[#bce1c7] rounded-full"
                      onClick={() => {
                        // Call existing functionality
                        setShowStatusCard("show_status_card");
                        handleViewTaskClick(item.status);
                        // Call new status click handler
                      }}
                    >
                      View Task
                    </div>
